...
//    · 영한 : 구글 번역 사전(비공식·키 불필요) → kengdic(data) → dictionaryapi.dev
...
// ── 사전 데이터 로드(모듈 1회) ── 번들/로컬 어디서든 찾도록 후보 경로 탐색
function resolveDataDir() {
  ...
  for (const d of cands) { try { if (fs.existsSync(path.join(d, "endic.json"))) return d; } catch { } }
  return cands[0];
}
const dataDir = resolveDataDir();
const ENDIC = JSON.parse(fs.readFileSync(path.join(dataDir, "endic.json"), "utf-8"));
const HANJA = JSON.parse(fs.readFileSync(path.join(dataDir, "hanja-chars.json"), "utf-8"));
const HW = JSON.parse(fs.readFileSync(path.join(dataDir, "hanja-words.json"), "utf-8"));
const WORD_IDX = {};
...
export function createApp({ store, env = process.env, serveStatic = false, publicDir } = {}) {
  ...
  async function searchEnglishFree(q) {
    const r = await fetchT(`https://api.dictionaryapi.dev/api/v2/entries/en/${encodeURIComponent(q)}`);
    if (r.status === 404) return [];
    if (!r.ok) throw new Error("dictionaryapi " + r.status);
    const data = await r.json();
    return (Array.isArray(data) ? data : []).slice(0, 2).map(e => {
      const phon = e.phonetic || (e.phonetics || []).map(p => p.text).find(Boolean) || "";
      return { headword: e.word, phon: phon ? `[${phon.replace(/^\/|\/$/g, "")}]` : "", speak: e.word, lang: "en-US",
        senses: (e.meanings || []).slice(0, 4).map(m => ({
          pos: m.partOfSpeech || "",
          defs: (m.definitions || []).slice(0, 3).map(d => d.definition),
          examples: (m.definitions || []).filter(d => d.example).slice(0, 2).map(d => ({ text: d.example })),
        })) };
    });
  }
  ...
  async function searchEnglish(q) {
    ...
    const k = searchEnglishKengdic(q);
    if (k.length) return k;
    return searchEnglishFree(q);
  }
  ...
  async function searchAncient(q) {
    ...
    catch (e) { if (/미설정/.test(e.message)) throw e; }
  ...
  async function hanjaWordLookup(t) {
    ...
    try {
      items = hasHan
        ? await uvalSearch({ q: t, advanced: "y", target: 2, method: "exact", num: 10 })
        : await uvalSearch({ q: t, advanced: "y", target: 1, method: "exact", type2: "chinese", num: 10 });
  ...
  function hanjaBrowse(t, mode) {
    ...
    for (const ch in HANJA) {
      ...
      const ok = mode === "hun"
        ...
        : (eum === t);
      if (ok) chars.push({ char: ch, read: HANJA[ch].read });
      if (chars.length >= 80) break;
    }
  ...
  async function synonymsEnglish(word) {
    const r = await fetchT(`https://api.datamuse.com/words?rel_syn=${encodeURIComponent(word)}&max=10`);
    if (!r.ok) return [];
    const data = await r.json();
